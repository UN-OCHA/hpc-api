...
import {
  AuthRequirementsGenerator,
  RequiredPermissionsCondition,
  RequiredPermissionsConditionOr,
} from "../../types/AuthRequirements";
import {
  GLOBAL_PERMISSIONS,
  OPERATION_CLUSTER_PERMISSIONS,
  OPERATION_PERMISSIONS,
} from "../../types/Permissions";
import ReportingWindowAssignment, {
  AssigneeType,
} from "../../entities/ReportingWindowAssignment";
import ReportingWindowAssignmentModel from "../../database/models/ReportingWindowAssignment";
...
import FileRecordModel from "../../database/models/FileRecord";
...
export default class ReportingWindowAssignmentResolver {
  static canViewAssignmentData: AuthRequirementsGenerator = async ({
    ...
    const assignment = await ReportingWindowAssignmentModel.findOne({
  ...
  static canEditAssignment: AuthRequirementsGenerator = async ({
    ...
    const canView = await ReportingWindowAssignmentResolver.canViewAssignmentData(
      { log, req, args }
    );
    const { assignment } = canView[0].data as {
      assignment: ReportingWindowAssignmentModel;
    };
  ...
  getAssignment(
    @Arg("assignmentId", { description: "ID of the assignment to get" })
    assignmentId: number,
    @Ctx() ctx: Context
  ) {
    return ReportingWindowAssignmentModel.findOneWithReportingWindow({
      id: Number(assignmentId) as any,
    });
  }
  ...
  async updateAssignment(
    @Arg("assignmentId", { description: "ID of the assignment to get" })
    assignmentId: number,
    @Arg("body", { description: "The body of the request" })
    ...
    const originalAssignment = await ReportingWindowAssignmentModel.findOne({
      id: Number(assignmentId) as any,
    });
    ...
    if (
      originalAssignment.versionModelInstance.version !== body.previousVersion
    ) {
      const error = {
    ...
    const currentFileHashes = new Set(
      (await FileRecordModel.listFilesInNamespace(storageNamespace)).map(
        (f) => f.hash
      )
    );
    const missingFileHashes = state.files?.filter(
      (fh) => !currentFileHashes.has(fh.data.fileHash)
    );
    ...
    const r = await originalAssignment.Update(
      a.versionModelInstance.version,
      a.versionModelInstance.data,
      ctx.participant.id
    );
    return r;
  }
